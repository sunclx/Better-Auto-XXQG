(() => {
  ...
  var require__ = __commonJS({
    "src/0.jsx"(exports, module) {
      ...
      var broadcast = TTXS_PRO_CONFIG.get("broadcast", "\u6C5F\u82CF\u65B0\u95FB\u5E7F\u64AD");
      var platform = TTXS_PRO_CONFIG.get("platform", "\u6C5F\u82CF\u5B66\u4E60\u5E73\u53F0");
      var subcolumn = TTXS_PRO_CONFIG.get("subcolumn", "\u603B\u4E66\u8BB0\u5728\u6C5F\u82CF");
      function google_ocr_api(img2) {
        console.log("GoogleMLKit\u6587\u5B57\u8BC6\u522B\u4E2D");
        let list = JSON.parse(JSON.stringify(gmlkit.ocr(img2, "zh").toArray(3)));
        let eps = 30;
        for (let i2 = 0; i2 < list.length; i2++) {
          for (let j = i2 + 1; j < list.length; j++) {
            if (list[i2]["bounds"]["bottom"] > list[j]["bounds"]["bottom"]) {
              let tmp = list[i2];
              list[i2] = list[j];
              list[j] = tmp;
            }
        ...
        for (let i2 = 0; i2 < list.length; i2++) {
          for (let j = i2 + 1; j < list.length; j++) {
            if (Math.abs(list[i2]["bounds"]["bottom"] - list[j]["bounds"]["bottom"]) < eps && list[i2]["bounds"]["left"] > list[j]["bounds"]["left"]) {
              let tmp = list[i2];
              list[i2] = list[j];
              list[j] = tmp;
            }
      ...
      function paddle_ocr_api() {
        ...
        let list = JSON.parse(JSON.stringify(paddle.ocr(arguments[0])));
        let eps = 30;
      ...
      if (ocr_choice == 2) {
        ...
        try {
          ...
          ocr.recognizeText = function(img2) {
            let results = this.detect(img2.getBitmap(), 1);
            let all_txt = "";
            for (let i2 = 0; i2 < results.size(); i2++) {
              let re = results.get(i2);
              all_txt += re.text;
            }
            return all_txt;
          };
      ...
      function do_shipin() {
        ...
        sleep(800);
        textMatches(/刷新重试|继续播放/).exists() && (fInfo("\u68C0\u6D4B\u5230\u6D41\u91CF\u63D0\u9192"), textMatches(/刷新重试|继续播放/).findOne().click());
        sleep(random(8e3, 9500));
        let re_times = 6;
        if (ddtong) {
          re_times += 6;
        }
        for (let i2 = 0; i2 < re_times; i2++) {
          click(device_w / 2, device_h / 2);
          sleep(500);
      ...
      function do_meizhou() {
        ...
        if (text("\u53D1\u73B0\u65B0\u7248\u672C").exists()) {
          return fError("\u6709\u5F39\u7A97\u65E0\u6CD5\u6BCF\u5468\u7B54\u9898\uFF0C\u53EF\u4F7F\u7528\u65E7\u7248\u4FEE\u6539\u7248\u672C\u53F7\u7248\u53D6\u6D88\u5F39\u7A97"), sleep(1e3), text("\u53D6\u6D88").findOne().click(), sleep(1e3), back(), text("\u6211\u8981\u7B54\u9898").waitFor(), sleep(1e3), back(), ran_sleep(), true;
        }
        ...
        if (meizhou_dao) {
          ...
          if (clt.empty()) {
            return fInfo("\u6BCF\u5468\u7B54\u9898\u5168\u90E8\u5DF2\u4F5C\u7B54\u3002"), ran_sleep(), back(), text("\u6BCF\u5468\u7B54\u9898").waitFor(), sleep(1e3), back(), text("\u6211\u8981\u7B54\u9898").waitFor(), sleep(1e3), back(), text("\u6211\u7684").waitFor(), ran_sleep(), true;
          }
          ...
          fInfo("\u6B63\u5E8F\u67E5\u627E\u672A\u505A\u9898\u76EE");
          let dixian_slt = text("\u60A8\u5DF2\u7ECF\u770B\u5230\u4E86\u6211\u7684\u5E95\u7EBF").filter(function(w2) {
            log("\u5E95\u7EBF\uFF1A", w2.bounds().top, device_h);
            return w2.bounds().top <= device_h - 30;
          });
          while (!text("\u672A\u4F5C\u7B54").exists()) {
            if (dixian_slt.exists()) {
              return fInfo("\u6BCF\u5468\u7B54\u9898\u5168\u90E8\u5DF2\u4F5C\u7B54\u3002"), back(), text("\u6BCF\u5468\u7B54\u9898").waitFor(), sleep(1e3), back(), text("\u6211\u8981\u7B54\u9898").waitFor(), sleep(1e3), back(), text("\u6211\u7684").waitFor(), ran_sleep(), true;
            }
            scoll.scrollForward();
            sleep(200);
          }
      ...
      function do_zhuanxiang() {
        ...
        var tihao = className("android.view.View").depth(24).findOnce(1).text();
        let reg3 = /(\d+) \/(\d+)/;
        var num = Number(tihao.match(reg3)[1]);
        var sum = Number(tihao.match(reg3)[2]);
        var substr = " /" + sum;
        while (num <= sum) {
          fClear();
          fInfo("\u7B2C" + num + "\u9898");
        ...
        if (textMatches(/\d+分/).findOne().text() != "100\u5206") {
          ...
          num = Number(tihao.match(reg3)[1]);
          sum = Number(tihao.match(reg3)[2]);
          substr = " /" + sum;
          sleep(1500);
          while (num <= sum) {
            text(num + substr).waitFor();
      ...
      function do_tiaozhan() {
        ...
        if (textStartsWith("total").exists()) {
          var a = true, b2 = className("android.widget.Image").textStartsWith("total").findOne().parent();
          ran_sleep();
          b2.click();
        ...
        for (ddtong && (max_total += 10); ; ) {
          ...
          try {
            ...
            var que_txt2 = className("android.widget.ListView").findOne().parent().child(0).text();
          } catch (p) {
            log("error1:", p);
            sleep(500);
            continue;
          ...
          if (text("\u7ED3\u675F\u672C\u5C40").exists()) {
            ...
            if (total < 5) {
              ...
              a && (back(), textStartsWith("total").waitFor(), sleep(2e3)), back(), text("\u767B\u5F55").waitFor();
              ran_sleep();
              return true;
            }
            total = 0;
      ...
      function do_duizhan1(renshu) {
        ...
        while (true) {
          ...
          for (let i2 = 0; i2 < 1; i2++) {
            ...
            console.time("\u9898\u76EE\u8BC6\u522B");
            if (ocr_choice == 0) {
              var que_txt2 = google_ocr_api(que_img).replace(
                /[^\u4e00-\u9fa5\d]|\d{1,2}\./g,
                ""
              );
            } else if (ocr_choice == 1) {
              var que_txt2 = paddle_ocr_api(que_img).replace(
                /[^\u4e00-\u9fa5\d]|\d{1,2}\./g,
                ""
              );
            } else {
              var que_txt2 = ocr.recognizeText(que_img).replace(
                /[^\u4e00-\u9fa5\d]|\d{1,2}\./g,
                ""
              );
            }
      ...
      function do_exec(type) {
        ...
        if (textStartsWith("\u5355\u9009\u9898").exists()) {
          let que_txt2 = className("android.view.View").depth(24).findOnce(1).parent().parent().child(1).text();
          ...
          if (ans2 && depth(26).text(ans2).exists()) {
            ...
            if (ans2 && reg3.test(ans2) && ans2.length == 1) {
              ...
              className("android.widget.RadioButton").findOnce(idx_dict[ans2[0]]).parent().click();
            } else {
              if (!ans2) {
          ...
          if (kong_num <= 1) {
            ...
            if (ans2 && word_num == ans2.length) {
              ...
              depth(25).className("android.widget.EditText").setText(ans2);
            }
          } else {
            if (type) {
              ans2 = get_ans_by_dati_tiku(que_txt2, type);
          ...
          let que_txt2 = className("android.view.View").depth(24).findOnce(1).parent().parent().child(1).text();
          let reg12 = /\s{3,}/g;
          let res2 = que_txt2.match(reg12);
          let collect = className("android.widget.CheckBox").find();
          if (res2.length == collect.length) {
            ans2 = "\u5168\u9009";
      ...
      function get_ans_by_re(que_txt) {
        ...
        if (tishi_txt.match(reg2)) {
          let ans2 = tishi_txt.match(reg2)[1];
          log(ans2);
          return ans2;
        } else {
          return 0;
      ...
      function get_ans_by_ocr1() {
        ...
        img2 = images.interval(img2, "#FD1111", 120);
        let ans2 = "";
        if (ocr_choice == 0) {
          ans2 = google_ocr_api(img2);
        } else if (ocr_choice == 1) {
          ans2 = paddle_ocr_api(img2);
        } else {
          ans2 = ocr.recognizeText(img2);
        }
        if (!ans2) {
          fInfo("\u672A\u8BC6\u522B\u51FA\u6587\u5B57");
        } else {
      ...
      function get_ans_by_http(que_txt2) {
        ...
        log(keyword);
        let req = http.get(
          "http://www.syiban.com/search/index/init.html?modelid=1&q=" + encodeURI(keyword)
        );
      ...
      function get_ans_by_dati_tiku(que_txt2, type) {
        ...
        for (let i2 = dati_tiku.length - 1; i2 >= 0; i2--) {
          let ti = dati_tiku[i2];
      ...
      function upload_wrong_exec(endstr) {
        ...
        if (textStartsWith("\u586B\u7A7A\u9898").exists()) {
          ...
          que_txt2 = className("android.view.View").depth(24).findOnce(1).parent().parent().child(1).text();
        }
      ...
      function jifen_init() {
        for (id("comm_head_xuexi_score").findOne().click(); !className("android.view.View").text("\u767B\u5F55").findOne(9e3); )
      ...
      function send_pushplus(token, sign_list) {
        ...
        for (let option of jifen_list.children()) {
          if ("old" == jifen_flag) {
            var title = option.child(0).child(0).text(), score = option.child(2).text().match(/\d+/g)[0], total = option.child(2).text().match(/\d+/g)[1];
          } else {
      ...
      function login(username, pwd) {
        ...
        if (begin_obj.text() == "\u767B\u5F55") {
          ...
          let b2 = className("EditText").id("et_pwd_login").findOne();
          a.setText(username);
          sleep(1e3);
          b2.setText(pwd);
          sleep(1e3);
      ...
      function refind_jifen() {
        className("android.webkit.WebView").scrollable().findOne().scrollForward();
        var a = className("android.widget.ListView").filter(function(b2) {
          return 8 < b2.rowCount();
        }).findOne();
      ...
      function entry_jifen_project(a) {
        var b2 = "old" == jifen_flag ? 3 : 4;
        jifen_list.findOne(textEndsWith(a)).parent().child(b2).click();
      }
      function winReshow() {
        for (i = 0; i < 4; i++) {
          recents();
      ...
      function noverify() {
        return threads.start(function() {
          for (; ; ) {
            ...
            fInfo("\u68C0\u6D4B\u5230\u6ED1\u52A8\u9A8C\u8BC1\uFF0C\u8BF7\u5C3D\u5FEB\u6ED1\u52A8");
            device.vibrate(a);
            textContains("\u5237\u65B0").exists() ? click("\u5237\u65B0") : textContains("\u7F51\u7EDC\u5F00\u5C0F\u5DEE").exists() ? click("\u786E\u5B9A") : sleep(1e3);
          }
        });
      }
      function fInit() {
      ...
      function xxqg(userinfo) {
        ...
        if (true == pinglun && ("old" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8BC4\u8BBA"]).child(2).text().match(
          /\d+/
        )[0]) || "new1" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8BC4\u8BBA"]).child(3).child(0).text() || "new2" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8BC4\u8BBA"]).child(3).text().match(/\d+/)[0]) {
          toastLog("\u5F00\u59CB\u8BC4\u8BBA");
          do_pinglun();
          jifen_list = refind_jifen();
        }
        if (true == shipin && ("old" == jifen_flag && "\u5DF2\u5B8C\u6210" != jifen_list.child(jifen_map["\u89C6\u9891"]).child(3).text()) || "old" != jifen_flag && "\u5DF2\u5B8C\u6210" != jifen_list.child(jifen_map["\u89C6\u9891"]).child(4).text()) {
        ...
        function qwdt() {
          if ("old" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8DA3\u5473\u7B54\u9898"]).child(2).text().match(
            /\d+/
          )[0] || "new1" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8DA3\u5473\u7B54\u9898"]).child(3).child(0).text() || "new2" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8DA3\u5473\u7B54\u9898"]).child(3).text().match(
            /\d+/
          )[0]) {
          } else {
            return;
        ...
        qwdt();
        if (true == wenzhang) {
          if ("old" == jifen_flag && "\u5DF2\u5B8C\u6210" != jifen_list.child(jifen_map["\u6587\u7AE0"]).child(3).text() || "old" != jifen_flag && "\u5DF2\u5B8C\u6210" != jifen_list.child(jifen_map["\u6587\u7AE0"]).child(4).text()) {
            console.verbose("\u65E0\u969C\u788D\u670D\u52A1\uFF1A" + auto.service);
            toastLog("\u5F00\u59CB\u6587\u7AE0\u6B21\u6570\u4E0E\u65F6\u957F");
            do_wenzhang();
            jifen_list = refind_jifen();
        ...
        if (true == bendi) {
          if ("old" == jifen_flag && "\u5DF2\u5B8C\u6210" != jifen_list.child(jifen_map["\u672C\u5730"]).child(3).text() || "old" != jifen_flag && "\u5DF2\u5B8C\u6210" != jifen_list.child(jifen_map["\u672C\u5730"]).child(4).text()) {
            toastLog("\u672C\u5730\u5F00\u59CB");
            do_bendi();
            jifen_list = refind_jifen();
          }
        ...
        if (0 != dingyue) {
          if ("old" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8BA2\u9605"]).child(2).text().match(
            /\d+/
          )[0] || "old" != jifen_flag && "0" == jifen_list.child(jifen_map["\u8BA2\u9605"]).child(3).child(0).text() || "new1" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8BA2\u9605"]).child(3).child(0).text() || "new2" == jifen_flag && "0" == jifen_list.child(jifen_map["\u8BA2\u9605"]).child(3).text().match(/\d+/)[0]) {
            toastLog("\u8BA2\u9605\u5F00\u59CB");
            d = do_dingyue();
            jifen_list = refind_jifen();
          }
        ...
        b = 1;
        if (2 != meizhou) {
          if (toastLog("\u6BCF\u5468\u7B54\u9898\u5F00\u59CB"), text("\u6211\u7684").findOne().click(), sleep(1e3), text("\u6211\u8981\u7B54\u9898").findOne(3e3)) {
            ...
            fError("V2.42\u53CA\u4EE5\u4E0A\u4E0D\u652F\u6301\u6BCF\u5468\u7B54\u9898"), back(), ran_sleep();
        }
        ...
        if (e = id("my_setting").findOne(3e3)) {
          ...
          sleep(6e3);
          do
            e = random(b.bounds().centerX(), b.bounds().right), c = b.bounds().centerY(), fInfo("\u70B9\u51FB\u8BBE\u7F6E\u6309\u94AE: " + e + "," + c), click(e, c);
      ...
      function main(userinfo) {
        ...
        for (let i2 = 0; i2 < 3; i2++) {
          fClear();
          fInfo("\u5F00\u59CB\u7B2C" + (i2 + 1) + "\u8F6E\uFF0C\u6700\u957F\u8FD0\u884C\u65F6\u95F4\u4E3A" + retry_time + "s");
