...
if (storage.get(engine_version, true)) {
  ...
  let gengxin_rows =
    "脚本有风险，仅供学习交流;更新内容：;1.原脚本会进入“我的”界面获取用户名，区分历史刷过文章，现取消此设定;2.可自定义滑动验证界面震动提醒时间;3.禁止截屏会随机选一个选项;4.自定义评论内容;脚本测试环境：强国V2.45.0;联系方式：tg: t.me/wyqg_ttxs;（点击取消不再提示）"
      .split(";");
...
function do_duizhan1(renshu) {
  ...
  while (true) {
    ...
    for (let i = 0; i < 1; i++) {
      ...
      if (ocr_choice == 0) {
        var que_txt = google_ocr_api(que_img).replace(
          /[^\u4e00-\u9fa5\d]|\d{1,2}\./g,
          "",
        );
      } else if (ocr_choice == 1) {
        var que_txt = paddle_ocr_api(que_img).replace(
          /[^\u4e00-\u9fa5\d]|\d{1,2}\./g,
          "",
        );
      } else {
        var que_txt = ocr.recognizeText(que_img).replace(
          /[^\u4e00-\u9fa5\d]|\d{1,2}\./g,
          "",
        );
      }
